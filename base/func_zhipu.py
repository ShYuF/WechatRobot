...
class ZhiPu():
    def __init__(self, conf: dict, role: dict) -> None:
    ...
    def get_answer(self, msg: str, wxid: str, **args) -> str:
        if msg != "":
            ...
            if answer != "":
                ...
                answer = self.role["error_input"]
            return answer
        else:
            return self.role["null_input"]

    def _update_message(self, wxid: str, msg: str, role: str) -> None:
        if wxid not in self.converstion_list.keys():
            self.converstion_list[wxid] = self.role["initial"].copy()
            
        content = {"role": role, "content": str(msg)}
        # self.converstion_list.append(content)
